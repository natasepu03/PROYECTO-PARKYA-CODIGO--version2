        ...
        Estado = r.Estado.ToString(),
           TarifaHora = t.precio_hora 
            ...
            var historialHoy = await (
                ...
                join u in _context.usuario on r.Usuario_id_usuario equals u.id_usuario
                 join t in _context.tarifas on v.Tipo_vehiculo_idTipo_vehiculo equals t.Tipo_vehiculo_idTipo_vehiculo
               where r.fecha.Date == hoy && (r.Estado == ParkYa.Models.Estado.Finalizada || r.Estado == ParkYa.Models.Estado.Cancelada)
                select new ReservaEmpleadoItemViewModel
                {
                    IdReserva = r.id_reserva,
                    CodigoReserva = r.cod_reserva,
            ...
            var historialSemana = await (
                ...
                join u in _context.usuario on r.Usuario_id_usuario equals u.id_usuario
                 join t in _context.tarifas on v.Tipo_vehiculo_idTipo_vehiculo equals t.Tipo_vehiculo_idTipo_vehiculo
                where r.fecha.Date >= inicioSemana && r.fecha.Date <= hoy && (r.Estado == ParkYa.Models.Estado.Finalizada || r.Estado == ParkYa.Models.Estado.Cancelada)
                select new ReservaEmpleadoItemViewModel
                {
                    IdReserva = r.id_reserva,
                    CodigoReserva = r.cod_reserva,
            ...
            var historialMes = await (
                ...
                join u in _context.usuario on r.Usuario_id_usuario equals u.id_usuario
                 join t in _context.tarifas on v.Tipo_vehiculo_idTipo_vehiculo equals t.Tipo_vehiculo_idTipo_vehiculo
              where r.fecha.Date >= inicioMes && r.fecha.Date <= hoy && (r.Estado == ParkYa.Models.Estado.Finalizada || r.Estado == ParkYa.Models.Estado.Cancelada)
                select new ReservaEmpleadoItemViewModel
                {
                    IdReserva = r.id_reserva,
                    CodigoReserva = r.cod_reserva,
...
public async Task<IActionResult> FinalizarReserva(int id, string horaEntrada, string horaSalida)
{
    ...
    // Guardar hora entrada si el empleado la ingresó
    if (!string.IsNullOrEmpty(horaEntrada) && TimeSpan.TryParse(horaEntrada, out TimeSpan horaEntradaTs))
    {
        reserva.hora_entrada = horaEntradaTs;
    }

    // Convertir horaSalida
    if (!TimeSpan.TryParse(horaSalida, out TimeSpan horaSalidaTs))
        return RedirectToAction("Reservas");

    var vehiculo = await _context.vehiculo.FirstOrDefaultAsync(v => v.id_vehiculo == reserva.Vehiculo_id_vehiculo);
    if (vehiculo == null)
        return RedirectToAction("Reservas");

    var tarifa = await _context.tarifas.FirstOrDefaultAsync(t => t.Tipo_vehiculo_idTipo_vehiculo == vehiculo.Tipo_vehiculo_idTipo_vehiculo);
    if (tarifa == null)
        return RedirectToAction("Reservas");

    // Calcular horas y monto
    var horas = (horaSalidaTs - reserva.hora_entrada.Value).TotalHours;
    if (horas < 0) horas = 0;
    ...
    TempData["Mensaje"] = $"Reserva finalizada. Total: {monto:C}";
